...
class CoarseHeapNoRefine(BasicSolver, ABC):
    ...
    def generate_coarse_grained_traj(self, query_id, dirs, query_feature, graph):
        ...
        self.candidate_path_num = path_num
        ...
        best_score_path = [(graph.top_k[node][0], graph.top_k[node][1]) for node in best_path]

        self.random_f1 = self.evaluate_single_path(best_score_path)
    ...
    def path_selection(self, query_id, dirs, query_feature, graph):
        self.longest_path_list = []
        self.time_path_selection, candidate_paths = self.generate_coarse_grained_traj(query_id, dirs,
                                                                                              query_feature, graph)
