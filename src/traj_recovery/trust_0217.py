...
class CoarseHeapNoRefine(BasicSolver, ABC):
    ...
    def generate_coarse_grained_traj(self, query_id, dirs, query_feature, graph):
                        ...
                        # 有多条边就开始计算 path 的得分
                            ...
                            if flag_gt:
                                if len(gt_path) == 0:
                                    ...
                                    gt_info.append([new_score, velocity_len, vision_len])
                                elif len(node_list_str) == len(gt_path[0]):
                                    gt_path.append(node_list_str)
                                    gt_info.append([new_score, velocity_len, vision_len])
                                elif len(node_list_str) > len(gt_path[0]):
                                    gt_path = [node_list_str]
                                    gt_info = [[new_score, velocity_len, vision_len]]
                                # logging.info("%s, score: %.3f, vel: %.3f, vis: %.3f" % (node_list_str, new_score, velocity_len, vision_len))

                            score_dic[str(temp)] = [new_score, velocity_len, vision_len]
        ...
        for idx in range(len(gt_path)):
            logging.info("%s, score: %.3f, vel: %.3f, vis: %.3f" % (str(gt_path[idx]), gt_info[idx][0], gt_info[idx][1], gt_info[idx][2]))
    ...
    def path_selection(self, query_id, dirs, query_feature, graph):
        self.longest_path_list = []
        self.time_path_selection, candidate_paths, scores = self.generate_coarse_grained_traj(query_id, dirs, query_feature, graph)
